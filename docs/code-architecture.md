...
로컬 클라이언트가 하위 프로세스로 실행하는 현재 배포 방식은 MCP 표준 STDIO 전송을 유지한다.
시스템 표준 입출력 접근은 조립 지점인 `Main.kt`에만 둔다.
`DoorayMcpServer`는 `kotlinx.io.Source`와 `kotlinx.io.Sink`를 주입받아 시스템 전역 입출력과 분리한다.

서버는 표준 출력에 MCP 프레임 외의 내용을 쓰지 않는다.
진단 로그는 표준 오류로만 보낸다.
실제 프로세스 연결 시험은 STDIO 전송으로 수행하고, 프로세스가 필요 없는 내부 시험은 `ChannelTransport`를 사용할 수 있다.

Streamable HTTP는 원격 또는 다중 클라이언트 배포가 필요할 때 별도 계획으로 검토한다.
도입할 때는 Origin 검증, 로컬 바인딩, 인증, 세션과 프록시 신뢰 경계를 함께 설계해야 한다.
...
등록기는 이름 중복과 프로필별 기대 도구 수를 시작 전에 검증한다.
구현이 덜 된 프로필은 빈 도구 목록이나 가짜 도구를 노출하지 않고 시작을 실패시킨다.
전환 기간의 기본값은 `legacy`이며 후속 도메인 계획이 12개 compact 모듈을 모두 채운 뒤 기본값을 바꾼다.
