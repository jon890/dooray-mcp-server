...
| 전역 `--json` | 모든 명령의 선택 출력 모드 | `structuredContent`와 `outputSchema`가 기본 구조화 결과를 제공한다. 호환용 `content`는 짧은 한국어 요약만 둔다. | 모든 compact 도구 | partial | legacy 성공·오류 구조화 결과와 실제 STDIO 오류 계약 시험 | CLI는 stdout JSON 모드다. MCP는 별도 옵션 없이 구조화 결과를 제공한다. legacy는 기존 JSON 텍스트를 보존하면서 `structuredContent`를 함께 반환하며 compact `outputSchema`는 후속 도메인 계획에서 연결한다. |
| 전역 `--quiet` | 식별자만 stdout 출력 | `result_mode`는 `ids`, `compact`, `full`을 지원한다. `ids`는 식별자 전용 결과다. | 모든 compact 도구 | partial | `ResultMode` 파싱과 `ids`의 `data` 생략 계약 시험 | 공통 실행 경계는 구현됐고 compact 도메인 도구 연결은 후속 계획에 남아 있다. |
| 전역 `--json --quiet` 동시 입력 | 공용 formatter에서는 JSON 우선 | MCP는 별도 출력 모드 충돌을 만들지 않는다. `result_mode` 하나만 허용하고 구조화 결과는 항상 유지한다. | 모든 compact 도구 | partial | 공통 메타 입력과 잘못된 결과 모드 시험 | 단일 열거형 계약은 구현됐고 도메인별 스키마 연결은 후속 계획에 남아 있다. |
| 전역 `--no-color`, `NO_COLOR` | ANSI 색상 비활성화 | MCP 결과에는 ANSI 색상 코드를 넣지 않는다. 로그도 터미널 색상에 의존하지 않는다. | 모든 도구와 서버 로그 | mcp-native-equivalent | ANSI 제거와 구조화 오류 공개 경계 시험 | MCP 클라이언트가 표시를 담당하므로 색상 옵션을 노출하지 않는다. |
| spinner와 진행 표시 | `--json` 또는 `--quiet`에서 비활성화 | STDIO stdout에는 MCP 프레임만 쓰고 진행 표시는 내보내지 않는다. 진단 로그는 stderr로 분리한다. | 서버 전송과 로깅 경계 | mcp-native-equivalent | 실제 STDIO 연결 9개와 `System.err` 로그 설정 검사 | MCP에서는 진행 표시를 출력하지 않고 프로토콜 프레임만 표준 출력으로 전송한다. |
| `stderr` | 진행 로그, 경고, 오류 문구 | 공개 결과의 `meta.warnings`, `error.details`, 서버 로그를 분리한다. 비밀 값과 stack trace는 공개 결과에 넣지 않는다. | `ToolExecutionBoundary` | partial | 안정 오류 코드, 비밀·스택·경로 제거, 상관관계 ID 시험 | 오류와 로그 경계는 구현됐고 도메인별 경고 매핑은 후속 계획에 남아 있다. |
...
| `--dry-run` | 일부 생성·수정·피드백에서 변경 계획 출력 | 변경 operation의 `dry_run=true`가 대상 해석, 요청 본문, 영향, 확인 토큰을 반환한다. | 모든 변경 compact operation | partial | 공통 `dry_run` 실행 문맥과 확인 저장소 시험, plan013 도구 연결 예정 | 공통 계약은 구현됐고 실제 변경 도구의 계획 생성은 후속 도메인 계획에 남아 있다. |
...
| 삭제 `-y/--yes` | TTY 확인 우회 | `prepare` operation이 삭제 계획과 짧은 만료의 `confirmation_token`을 반환하고, `confirm` operation이 같은 대상인지 검증한 뒤 실행한다. | 삭제 대상별 compact 도구 | partial | 해시 토큰, 5분 만료, 대상 결합, 단일 소비 저장소 시험과 plan013 도구 연결 예정 | 안전 저장소 기반은 구현됐지만 현재 legacy 삭제는 즉시 실행되므로 실제 차단 연결은 plan013에서 완료한다. |
