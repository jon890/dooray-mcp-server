            ...
            ToolSchema(
                properties =
                    buildJsonObject {
                        ...
                        putJsonObject("type") {
                            put("type", "string")
                            put("description", "프로젝트 타입 (public: 일반 프로젝트, private: 개인 프로젝트, 기본값: public)")
                            put("default", "public")
                        }
                        putJsonObject("scope") {
                            put("type", "string")
                            put("description", "접근 범위 (private: 프로젝트 멤버만 접근, public: 조직 멤버 누구나 접근, 기본값: private)")
                            put("default", "private")
                        }
                        putJsonObject("state") {
                            put("type", "string")
                            put("description", "프로젝트 상태 (active: 활성화, archived: 보관됨, deleted: 삭제됨)")
                        }
                    }
            ),
        outputSchema = null,
            ...
            if (response.header.isSuccessful) {
                ...
                successResult(
                    data = buildJsonObject {
                        put("projects", JsonUtils.toJsonElement(response.result))
                    ...
                    message = "프로젝트 목록을 성공적으로 조회했습니다 (총 ${response.totalCount}개 중 ${response.result.size}개 조회)"
                )
            } else {
